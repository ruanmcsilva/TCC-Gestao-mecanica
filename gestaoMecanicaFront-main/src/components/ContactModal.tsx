...
interface ContactModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void; // Nova prop para abrir o sucesso
}
...
const ContactModal: React.FC<ContactModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSuccess(); // Chama a função que fecha este e abre o de sucesso
  };
      ...
      <div className="relative bg-white p-8 rounded-[20px] shadow-2xl max-w-md w-full transform transition-all flex flex-col items-center">
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-6">
          ...
          <div className="flex flex-row w-full gap-4 pt-4">
            ...
            <button
              type="submit"
              className="w-1/2 py-2 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-full transition-all shadow-md active:scale-95 shadow-amber-500/20"
            >
              Enviar
            </button>
          </div>
